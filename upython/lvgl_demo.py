###############################################################################
#  LVGL 9.3 — Comprehensive Widget & Feature Demo
#  ─────────────────────────────────────────────────────────────────────────────
#  Target  : STM32H743  ·  32 MB SDRAM  ·  1024×600 LCD  ·  Capacitive Touch
#  Stack   : MicroPython + LVGL 9.3 (bindings already ported)
#  Author  : Generated demo — adapt as needed
#
#  IMPORTANT: This file assumes the display driver and touch driver are already
#  initialised before this script runs.  Only LVGL widget‑level code is used.
###############################################################################

# ═════════════════════════════════════════════════════════════════════════════
#  LANGUAGE SETTING  — change to "it" for Italian, "en" for English
# ═════════════════════════════════════════════════════════════════════════════
LANG = "en"                       # <<< SET LANGUAGE HERE: "en" | "it"
...
_S = {
    ...
    "welcome_sub":    {"en": "Comprehensive Widget Demo\n1024 x 600 · 32 MB SDRAM · Capacitive Touch",
                       "it": "Demo Completa dei Widget\n1024 x 600 · 32 MB SDRAM · Touch Capacitivo"},
    "home_info":      {"en": "Swipe tabs to explore all features.\n\n"
                       ...
                       "it": "Scorri le schede per esplorare.\n\n"
...
def T(key):
    """Return the translated string for the current LANG."""
...
# ═════════════════════════════════════════════════════════════════════════════
#  HELPER: Create a styled panel / card inside a parent
# ═════════════════════════════════════════════════════════════════════════════
...
def setup_tab_bg(tab):
    """Apply a consistent dark background and padding to a tab page."""
...
def build_home(tab):
    ...
    try:
        ...
        pass  # font not compiled in — use default
...
def build_data(tab):
    ...
    # Timer to animate progress bars
    prog_state = [0]
...
# ═════════════════════════════════════════════════════════════════════════════
#  TAB 4  —  SELECTION  (dropdown, roller, table, list)
# ═════════════════════════════════════════════════════════════════════════════
...
def build_text(tab):
    ...
    # Focus handler — assign keyboard to the focused textarea
    def on_ta_focus(e):
        kb.set_textarea(ta)
...
def build_anim(tab):
    ...
    # ── Animated button‑matrix (colour cycling) ─────────────────────────────
    ...
    # Create 8 small coloured boxes in a row
...
def build_styles(tab):
    ...
    l0 = lv.label(c0)
    l0.set_text(titles[0])
    l0.set_style_text_color(COLOR_WHITE, 0)
    ...
    l1 = lv.label(c1)
    l1.set_text(titles[1])
    l1.set_style_text_color(COLOR_PRIMARY, 0)
    ...
    l2 = lv.label(c2)
    l2.set_text(titles[2])
    l2.set_style_text_color(COLOR_TEXT, 0)
    ...
    l4 = lv.label(c4)
    l4.set_text(titles[4])
    l4.set_style_text_color(COLOR_BG_DARK, 0)
    ...
    l5 = lv.label(c5)
    l5.set_text(titles[5])
    l5.set_style_text_color(COLOR_TEXT_DIM, 0)
    ...
    section_label(card_play, "Radius & Opacity" if LANG == "en"
                  else "Raggio & Opacità")
...
# ═════════════════════════════════════════════════════════════════════════════
#  MAIN — Build the tabview and all tabs
# ═════════════════════════════════════════════════════════════════════════════
def main():
    # ── Get active screen (verified: lv.screen_active() works) ───────────────
    scr = lv.screen_active()
    scr.set_style_bg_color(COLOR_BG_DARK, 0)

    # ── Create tabview (1-arg constructor verified working) ─────────────────
    tv = lv.tabview(scr)
    try:
        tv.set_tab_bar_position(lv.DIR.TOP)
        tv.set_tab_bar_size(48)
    except Exception:
        pass  # some builds configure via constructor only
    ...
    print("[LVGL DEMO] running — lang={}, screen=1024x600".format(LANG))
